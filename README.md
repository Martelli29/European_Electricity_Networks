...
the nodes . The file "share-elec-produc-by-source.txt" provides the
contribution of each type of electricity generation for each state,
which I used to calculate the carbon density . I applied six different
...
carbon intensity of each source of energy is taken from IPCC .
...
identified using the Louvain method ), communities are logged to the
...
it’s implemented on the library of NetworkX .
...
corresponding NetworkX function ) is calculated by measuring the amount
...
corresponding NetworkX function ) has the same aim of the current flow
betweenness centrality but for the edges.

The current flow closeness (computed thanks to corresponding NetworkX
function ) centrality is variant of closeness centrality based on
...
function ) provides insights into the overall influence or significance
